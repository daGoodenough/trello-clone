...
function CardLocation ({setIsDeletingCard, index, cards, setCards, description, setIsPostingCardDetails, listId}){
...
return (
    ...
    {cards.map((i) => {
      if (i.listId === listId && i.order === index) {
        return <Card setIsDeletingCard={setIsDeletingCard} order={index} key={i.id} title={i.title} cardId={i.id} listId={i.listId} description={i.description} workflow={description} comments={i.comments} setIsPostingCardDetails={setIsPostingCardDetails}/>
      }
      if(i.order === index){
